...
//    User 0 can see the labels only of even numbered tiles, unless the tile is above the line.
//    User 1 can see the labels only of odd numbered tiles, unless the tile is above the line.
   ...
   let g3 = new G3(model, draw => {
      ...
      if (p = p3S.p)                              // RENDER WORD TILES AFTER DATA HAS LOADED.
         for (let n = 0 ; n < p.length ; n++) {   // IF A TILE IS MY GENDER OR IS IN THE SHARED
            ...
            let tileColor = sharedTile ? 'white' : tileGender ? blue : pink;
